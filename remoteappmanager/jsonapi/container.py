...
from remoteappmanager.jsonapi.application import ApplicationSchema
...
class ContainerSchema(Schema):
    ...
    mapping_id = fields.String(required=True, load_only=True)
    configurables = fields.Dict(load_only=True)
    application = fields.Relationship(
        self_url="/containers/{container_id}/relationships/application",
        self_url_kwargs={"container_id": "<id>"},
        related_url="/applications/{application_id}/",
        related_url_kwargs={"application_id": "application.id"},
        dump_only=True,
...
class ContainerList(ResourceList):
    ...
    model_connector = ContainerConnector


class ContainerDetails(ResourceDetails):
    schema = ContainerSchema
    model_connector = ContainerConnector
...
class ContainerConnector(ModelConnector):
    @gen.coroutine
    @authenticated
    def create_object(self, data, **kwargs):
        ...
        if not choice:
            ...
            raise exceptions.BadRequest.from_message("unrecognized mapping_id")

        _, app, policy = choice[0]

        image = yield container_manager.image(app.image)
        if image is None:
            raise exceptions.BadRequest.from_message("unrecognized image")

        try:
            environment = self._environment_from_configurables(image, data)
        except Exception:
            self.log.exception("Invalid configurables")
            raise exceptions.BadRequest.from_message("invalid configurables")
        ...
        try:
            ...
            raise exceptions.Unable.from_message(str(e))
    ...
    def _environment_from_configurables(self, image, data):
        ...
        requested_configurables = data.get("configurables")
        if requested_configurables is None:
            return env

        for img_conf in image.configurables:
            config_dict = requested_configurables.get(img_conf.tag)
